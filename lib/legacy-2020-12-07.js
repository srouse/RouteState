...
RouteState.serializePathname = true;
...
RouteState.valueSeparator = "/";
RouteState.nameSeparatorStart = "{";
RouteState.nameSeparatorEnd = "}";
RouteState.nameSeparator = ",";
RouteState.valueNameSeparator = RouteState.valueSeparator + RouteState.nameSeparatorStart;
RouteState.dependancySeparatorStart = "[";
RouteState.dependancySeparatorEnd = "]";
RouteState.nameRelationSeparator = RouteState.nameSeparatorEnd + RouteState.dependancySeparatorStart;
RouteState.dependancyConnectionSeparator = ":";
RouteState.dependancySeparator = ",";
...
RouteState.objectFromPath = function (path) {
  ...
  // Get rid of shebang
  path = path.replace(/#!\//g, '');
  path = path.replace(RouteState.dependancySeparatorEnd, '');

  var dependencyArr = path.split(RouteState.nameRelationSeparator);
  ...
  var pathArr = path.split(RouteState.valueNameSeparator);
  if (pathArr.length < 2) {
    return routeStateRoute;
  }
  ...
  for (var a = 0; a < namesArr.length; a++) {
    ...
    if (dependency) {
      ...
      depnameArr = dependency.split(RouteState.dependancyConnectionSeparator );
      if (depnameArr.length > 1) {
        dependency = namesArr[depnameArr[0]] + RouteState.dependancyConnectionSeparator  + depnameArr[1];
      } else {
...
RouteState.stateToObject = function (state) {
  ...
  if (state.indexOf(RouteState.dependancyConnectionSeparator) !== -1) {
    stateArr = state.split(RouteState.dependancyConnectionSeparator);
...
RouteState.tieToPropAndValue = function (source, target, overrides) {
  ...
  if (
    ...
    RouteState.config[source].dependency =
      target + RouteState.dependancyConnectionSeparator + targetValue;
  } else {
...
RouteStateRoute.prototype.toHashString = function () {
  ...
  routeArr.forEach(function(value) {
    if (
      RouteState.config &&
      RouteState.config[value.name] &&
      RouteState.config[value.name].dependency
    ) {
      depnameArr = RouteState.config[value.name].dependency.split(RouteState.dependancyConnectionSeparator);
      if (depnameArr.length > 1) {
        dependancyArr.push(nameLookup[depnameArr[0]] + RouteState.dependancyConnectionSeparator  + depnameArr[1]);
  ...
  if (valArr.length > 0) {
    if (showDependencies) {
      return '#!/' + valArr.join('/') + RouteState.valueNameSeparator + nameArr.join(RouteState.nameSeparator) + RouteState.nameRelationSeparator + dependancyArr.join(RouteState.dependancySeparator) + RouteState.dependancySeparatorEnd;
    }
    return '#!/' + valArr.join('/') + RouteState.valueNameSeparator + nameArr.join(RouteState.nameSeparator) + RouteState.nameSeparatorEnd;
  }
  return '';
};
...
RouteStateRoute.prototype.serializedToBodyClasses = function (prefix) {
  ...
  if (RouteState.serializePathname && RouteState.targetDocument !== undefined) {
    bodyClasses.push(
  ...
  for (var name in this) {
    if (!RouteState.isFunction(this[name])) {
      ...
      if (routeConfig && typeof routeConfig.dependency !== 'undefined') {
        ...
        dependencyArr = routeConfig.dependency.split(RouteState.dependancyConnectionSeparator);
        if (dependencyArr.length > 1) {
